...
const CoursePlayer = () => {
  ...
  // FIXED: Initialize current lecture index based on navigation state or progress
  useEffect(() => {
    if (course.lectures && course.lectures.length > 0 && !hasInitialized) {
      let startingIndex = 0;
      
      console.log('Navigation state:', location.state);
      console.log('Course lectures:', course.lectures);
      
      // Check if we have a specific lecture to start from in navigation state
      if (location.state?.startFromLectureId) {
        const startLectureId = location.state.startFromLectureId;
        const lectureIndex = course.lectures.findIndex(lecture => lecture.id === startLectureId);
        console.log('Looking for lecture ID:', startLectureId, 'Found at index:', lectureIndex);
        
        if (lectureIndex !== -1) {
          startingIndex = lectureIndex;
          console.log('✅ Starting from specific lecture at index:', startingIndex, 'Title:', course.lectures[startingIndex]?.title);
        } else {
          console.log('❌ Lecture ID not found, falling back to progress-based selection');
        }
      }
      
      // If no specific lecture from navigation, find first incomplete lecture
      if (startingIndex === 0 && location.state?.startFromLectureId === undefined) {
        const savedCompleted = localStorage.getItem(`user_${currentUser._id}_course_${courseId}_completed_lectures`);
        const completedSet = savedCompleted ? new Set(JSON.parse(savedCompleted)) : new Set();
        
        const incompleteIndex = course.lectures.findIndex(lecture => !completedSet.has(lecture.id));
        console.log('First incomplete lecture index:', incompleteIndex);
        
        if (incompleteIndex !== -1) {
          startingIndex = incompleteIndex;
          console.log('✅ Starting from first incomplete lecture at index:', startingIndex, 'Title:', course.lectures[startingIndex]?.title);
        } else {
          startingIndex = 0;
          console.log('✅ All lectures completed, starting from first');
        }
      }
      
      setCurrentLectureIndex(startingIndex);
      setHasInitialized(true);
      setVideoError(false);
      setVideoLoading(true);
      
      console.log('🎯 Final starting index:', startingIndex, 'Lecture:', course.lectures[startingIndex]?.title);
    }
  }, [course.lectures, courseId, currentUser, location.state, hasInitialized]);
