...
const Dashboard = () => {
  ...
  const [showReviewsModal, setShowReviewsModal] = useState(false); // NEW: Reviews modal state
  ...
  // FIXED: Improved progress calculation that persists completed lectures
  const calculateCourseProgress = (course, userId) => {
    if (!course || !userId) return 0;
    
    // Get completed lectures from localStorage
    const completedLecturesKey = `user_${userId}_course_${course._id}_completed_lectures`;
    const savedCompleted = localStorage.getItem(completedLecturesKey);
    
    if (savedCompleted && course.lectures && course.lectures.length > 0) {
      const completedLectures = JSON.parse(savedCompleted);
      const progress = (completedLectures.length / course.lectures.length) * 100;
      return Math.round(progress);
    }
    
    // Fallback to old progress calculation if no completed lectures found
    const userProgress = JSON.parse(localStorage.getItem(`user_progress_${userId}_${course._id}`) || '{}');
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const enrolledCoursesData = validCourses.filter(course => 
        ...
        // FIXED: Use the improved progress calculation that persists completed lectures
        const progress = calculateCourseProgress(course, currentUser._id);
        ...
        return {
          ...course,
          progress: progress, // Use calculated progress instead of userProgress.progress
  ...
  const generateRecentActivity = (courses, user) => {
    ...
    if (courses.length > 0) {
      ...
      // FIXED: Show actual progress from persisted data
      ...
      // Add activity for completed courses
      const completedCourse = courses.find(course => course.progress === 100);
      if (completedCourse) {
        activities.push({
          type: 'completion',
          message: `You completed "${completedCourse.title}"! 🎉`,
          time: 'Recently',
          icon: '✅'
        });
      }
  ...
  // NEW: Function to handle viewing reviews in a modal
  const handleViewReviews = (course) => {
    if (!course.ratings || course.ratings.length === 0) {
      alert('No reviews available for this course.');
      return;
    }
    setSelectedCourse(course);
  ...
  const handleUnregisterCourse = (courseId) => {
    ...
    // Remove user progress and completed lectures
    localStorage.removeItem(`user_progress_${currentUser._id}_${courseId}`);
    localStorage.removeItem(`user_${currentUser._id}_course_${courseId}_completed_lectures`);
  ...
  return (
    <div style={styles.container}>
      ...
      {/* View Reviews Modal - NEW */}
      {showReviewsModal && (
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            <div style={styles.modalHeader}>
              <h2 style={styles.modalTitle}>
                Reviews for {selectedCourse?.title}
              </h2>
              <button 
            ...
            <div style={styles.reviewsModalContent}>
              {selectedCourse?.ratings && selectedCourse.ratings.length > 0 ? (
                <div style={styles.reviewsContainer}>
                  <div style={styles.reviewsSummary}>
                    <div style={styles.averageRating}>
                      <span style={styles.averageRatingNumber}>
                        {selectedCourse.averageRating?.toFixed(1) || '0.0'}
                      </span>
                      <div style={styles.averageRatingStars}>
                        {renderStarRating(Math.round(selectedCourse.averageRating || 0))}
                      </div>
                      <span style={styles.totalReviews}>
                        Based on {selectedCourse.ratings.length} review{selectedCourse.ratings.length !== 1 ? 's' : ''}
                      </span>
                    </div>
                  </div>

                  <div style={styles.reviewsList}>
                    {selectedCourse.ratings.map((review, index) => (
                ...
                <div style={styles.noReviews}>
                  ...
                  <p style={styles.noReviewsText}>
                    This course doesn't have any reviews yet. Students will be able to leave reviews after completing the course.
                  </p>
                </div>
              )}
            </div>
                    ...
                    <div style={styles.courseCardContent}>
                      ...
                      {course.ratings && course.ratings.length > 0 && (
                        <div style={styles.reviewsSection}>
                          ...
                          >
                            <span style={styles.reviewsIcon}>⭐</span>
                            View Reviews ({course.ratings.length})
                          </button>
                        </div>
                      )}
